...
<!--
*   For Beta 1 releases, it is strongly recommended to coordinate with the Outreach folks in [#outreach](https://make.wordpress.org/core/tag/outreach/) to publish and promote a “Help Test WordPress X.Y” post (e.g., [6.1 post](https://make.wordpress.org/test/2022/09/21/help-test-wordpress-6-1/), [6.1 Slack message](https://wordpress.slack.com/archives/C015GUFFC00/p1663781207569069)).
...
*   Bump Version.
    *   Update the `$wp_version` in `trunk/src/wp-includes/version.php`  
        ...
        [![](https://make.wordpress.org/core/files/2017/06/pasted-image-0-1-1024x547.png)](https://make.wordpress.org/core/files/2017/06/pasted-image-0-1.png)
    ...
    *   Ensure the version bump appears on [https://build.trac.wordpress.org/](https://build.trac.wordpress.org/). *This only needs to be verified by the person releasing via Mission Control*.
*   Build the packages.
...
*   リリースパーティに参加する人たちに、テストが終わるまでリリースパッケージへのリンクを共有しないよう注意を促してください。
    *   例 ([アーカイブ](https://wordpress.slack.com/archives/C02RQBWTW/p1584479009394600)): **:siren: Reminder: Please do not share links to the package publicly until the all-clear is given after testing and the announcement post is published on WordPress.org. :siren:**
...
*   最新の GitHub Action チェックがパスしていることを確認します (つまり、すべて ✅ が表示されます)。
    *   [https://github.com/WordPress/wordpress-develop/actions?query=event%3Apush+branch%3Atrunk](https://github.com/WordPress/wordpress-develop/actions?query=event%3Apush+branch%3Atrunk) の最新コミットをチェックします。
...
*   バージョンを上げます。
    *   `trunk/src/wp-includes/version.php` の `$wp_version` を更新します
        [![](https://make.wordpress.org/core/files/2017/06/pasted-image-0-1-1024x547.png)](https://make.wordpress.org/core/files/2017/06/pasted-image-0-1.png)
...
*   パッケージをビルドします。
    ...
    *   パッケージ化されたら、手動でアップデートをテストするなど、十分にテストする必要があります。(その方法は ? [ドキュメント](https://ja.wordpress.org/team/handbook/core/about/release-cycle/releasing-minor-versions/#testing-packages)をチェックしてください)。
    *   ビルド名を入力しましたか ?
    *   ボタンをクリックしましたか ?
*   パッケージをテストします。
    *   URL (例: [https://wordpress.org/wordpress-4.9-beta2.zip](https://wordpress.org/wordpress-4.9-beta2.zip)) と、テストすべき具体的な項目をハイライトしたテスト募集の投稿 ([6.8 の例](https://make.wordpress.org/test/2025/03/04/help-test-wordpress-6-8/)) を共有して、[テスト](https://ja.wordpress.org/team/handbook/core/about/release-cycle/releasing-minor-versions/#testing-packages)を依頼してください。パッケージのテストに協力する方法は3つあります:
        *   [WordPress Beta Tester](https://wordpress.org/plugins/wordpress-beta-tester/) プラグインをインストールし、有効化します
            *   メジャー RC リリースの場合は、`Bleeding edge` チャンネルを選択し、次に `Beta/RC Only` ストリームを選択します。マイナーな RC リリースの場合は、Point Release チャンネルと Nightlies ストリームを選択してください (Nightlies がビルドされるまでは、ベータテスターを使ってマイナーなベータ/RC パッケージをテストできません)。
...
[#fse-outreach-experiment](https://make.wordpress.org/core/tag/fse-outreach-experiment/), [#props](https://make.wordpress.org/core/tag/props/)
