...
*   ハンドブックの締め切りに目を配り、関係するさまざまなチームに連絡を取る (プラグイン、テーマ、Polyglots などに通知を送る)
*   期限を知らせる
*   ハンドブックに概説されている手順をメモし、必要であれば変更を提案する。
*   必要であれば、チーム間の議論を促進する
...
*   すべての貢献者がリリースサイクルの段階に従っていることを確認する。たとえば、例外が提案され、プロジェクトリーダーによって承認されない限り、ベータ版の間は機能拡張を行わない。
*   ベータ、RC、最終リリースの各プロセスが、技術面で可能な限りスムーズに実行され、すべてのリリース手順プが完了していることを確認する
...
*   開発ノートを必要とするリリース内の変更点を追跡する
*   変更点を最もよく理解しているチケットの参加者 (コミッター、コンポーネントメンテナー、チケットを所有する貢献者、担当者) と調整し、開発ノートの下書きを作成する
*   すべての開発ノートが、(リリース候補1と同時に発行される) フィールドガイドの前に、校正、レビュー、公開するために十分な時間をかけて作成されていることを確認する
*   チケット参加者が開発ノートを書けない場合、書いてくれる人を見つけるか、自分で書く
...
*   ドキュメント調整チームから提供される開発ノートを校正・レビューする
*   コード例を確認する
*   追加の例を提案する
*   開発ノートが問題、解決策を正確かつ徹底的に記述し、変更点の適切な使用方法を特定していることを確認する
...
*   Trac チケットを通して、Gutenberg とコアの同期・更新を手助けする
...
*   主にフロントエンドで重要な点に冠詞て、コードでテーマに貢献する
...
*   すべてのデフォルトテーマが現在のリリースで追加された新機能を完全にサポートしていることを保証する (適切と判断された場合)。
